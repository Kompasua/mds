...
\begin{document}
    ...
    Pr"ufen wir ob das f"ur $n+1$ gilt. Wir hinzufügen zu jeden Fall ein Knoten. Bekommen wir:\\\\
    1. 
	\begin{tikzpicture}
      	[scale=.8,auto=left,every node/.style={circle,fill=blue!20}]
       	\node (n1) at (0,0) {};
    ...
    \begin{tikzpicture}
    	...
    	\node (n5) at (1,1) {};
    \end{tikzpicture}\\\\
    Da die Graphen aus vorheriger Beispiel Teilgraphen von dieses Beispiel sind, das gilt auch f"uer andere Anzahl der Knoten. Sofern kann mann sehen, dass induzierter Graph mit $V=4$ aus vollständiger Graph vieder ein 
    vollständiger Graph ist und nach Definition der Kreis, kann Kreis bilden kann. In zweiten Beispiel unabh"angige Knoten
    bleiben nicht verbunden und damit kann man auch kein Kreis bilden. Im dritten Fall es ist augenfällig, dass es kein 
    Kreis sein kann. 
